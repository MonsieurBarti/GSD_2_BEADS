...
export function runBdCreate(args: string[]): string {
...
export function createTask(
  ...
  const args: string[] = ["--type=task", `--title=${title}`, `--description=${description}`];
  if (opts.acceptance_criteria) {
  ...
  if (opts.parentId) {
    args.push(`--parent=${opts.parentId}`);
  }
  if (opts.depIds && opts.depIds.length > 0) {
    args.push(`--deps=${opts.depIds.join(",")}`);
  }
  return runBdCreate(args);
}
